...
const fileName = computed(() => {
  // code 为自定义文件名（非长代码模板）时优先显示
  const code = props.image.code
  if (code && code.length < 100 && !code.includes('function')) {
    return code
  }
  const d = new Date(props.image.timestamp)
  const pad = (n: number) => String(n).padStart(2, '0')
  return `截图 ${d.getMonth() + 1}-${pad(d.getDate())} ${pad(d.getHours())}:${pad(d.getMinutes())}:${pad(d.getSeconds())}`
})

const downloadName = computed(() => {
  const name = fileName.value
  return name.includes('.') ? name : `${name}.${ext.value}`
})
